...
​  * 1. ​通信接口​​：
  *      支持 ​​I2C 协议​​（仅限 I2C）
  *      默认 I2C 地址：0x0E（可通过硬件配置修改）
​  * 2. ​供电要求​​
  *      工作电压：​​1.8V–3.3V​​（需与主控芯片电平匹配）
  * @section 使用须知
  *      需调用 ist8310_init() 完成软复位、寄存器配置及设备 ID 校验等初始化操作
  *      调用 ist8310_read_mag(fp32 mag[3]) 函数读取地磁场数据
  * @section 数据转换公式
  *      地磁场(uT) = RAW_DATA × MAG_SEN
  * 
  *
  @verbatim
  ==============================================================================
...
void Send_ist8310_To_Vofa(UART_HandleTypeDef *huart,float mag[3])
{
  char buffer[128]; // 缓冲区
  int length = 0;
