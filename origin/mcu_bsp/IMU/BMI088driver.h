...
/**
 * @note：用以存储BMI反馈的数据，方便调用
 */
typedef struct BMI_FEEDBACK_DATA
{
    fp32 g_x;
    fp32 g_y;
    fp32 g_z;
...
extern void Send_BMI088_To_VOFA(UART_HandleTypeDef *huart,float gyro[3], float accel[3], float temp);

// 采样数据的存储地，分别存储了三轴陀螺仪 三轴加速度计 和温度数据
